...
from TAS_RL.memory import ReplayMemory
import copy
    ...
    def agent_training(self):
        ...
        for n in range(self.no_of_training_episode):
            ...
            if (len(self.victim_critic_memory) > self.cfg.epoch*self.cfg.batch_size) and (n%10==0):
                ...
                agent_reward_path = os.path.join(plot_dir, 'agent_reward_plot')
                self.plot_reward(victim_reward_vec, agent_reward_path)   
                    
                agent_critic_loss_path = os.path.join(plot_dir, 'agent_critic_loss_plot')
                agent_policy_loss_path = os.path.join(plot_dir, 'agent_policy_loss_plot')
