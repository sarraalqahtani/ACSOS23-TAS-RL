...
from TAS_RL.memory import ReplayMemory, ConstraintReplayMemory
import copy
...
#******************************************************************************************************************
    def __init__(self, env, cfg, test_env):
      ...
      #**************************************************************
        ...
        wandb.init(project='[torch] TASRL_VAE_MuJoCo')
        self.safety_agent = Safety_Agent_VAE(observation_space = self.agent_observation_space, 
            ...
            if (len(self.safety_constrained_memory) > self.rec_training_epoch*self.cfg.batch_size) and (n>=self.cfg.train_start):
                ...
                if self.VAE:
                    ...
                    for i in range(self.rec_training_epoch):
                        safety_policy_loss, critic_loss, vae_ls  = self.safety_agent.update_parameters(self.safety_constrained_memory, self.cfg.batch_size)
                        safety_pol_loss+=safety_policy_loss
                        safety_crtc_loss+=critic_loss
                        VAE_loss+=vae_ls
                    safety_pol_loss = safety_pol_loss/self.rec_training_epoch
                    safety_crtc_loss = safety_crtc_loss/self.rec_training_epoch
                    VAE_loss = VAE_loss/self.rec_training_epoch
                    log_data = {"safety":safety, 'safety ratio':safety_ratio, "policy loss":safety_pol_loss, "critic loss":safety_crtc_loss, "vae loss":VAE_loss}
                    print(log_data)
                    wandb.log(log_data)
                else:
                    safety_pol_loss = 0  
                    ...
                    log_data = {"safety":safety, 'safety ratio':safety_ratio, "policy loss":safety_pol_loss}
                    print(log_data)
                    wandb.log(log_data)
  ...
  #==============================================================
    def safety_trajectory(self, env, init_state):
        ...
        if self.cfg.env_name =="maze":
            state =env.reset(pos=init_state)
        ...
        while not done:
            ...
            episode_reward+=reward
            mask = float(not done)
            self.safety_constrained_memory.push(state, action, reward, float(info['constraint']), next_state, mask)
            state = next_state
