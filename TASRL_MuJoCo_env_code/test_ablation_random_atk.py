...
from plot_scripts.plot_results import *
import warnings
      ...
      #******************************************************************************   
        if not safety_agent==None:
            ...
            if shield_val_tsk>=shield_threshold:
                action = safety_agent.select_ablation_action()
                rec_cnt+=1
            else:
                action = action_tsk 
                tsk_cnt+=1     
...
def run(env_name=None, eval_epi_no=100):
  ...
  TAS_RL_data_path = os.path.join(expdata_path, env_name, exp_atk_name,'TAS_RL','Ablation','shield_threshold_{}'.format(shield_threshold))
  #====================================================================
  for eps in epsilon_list:
        ...
        RecRLexp_data_path = os.path.join(expdata_path, env_name, exp_atk_name, 'RecRL', 'Ablation')
        ...
        #**********************************************************************************************************
        #**********************************************************************************************************
  # atk_name = 'random/ablation'
  plot_path = os.path.join(expdata_path, env_name, 'Plots', exp_atk_name)
  get_result_data(env_name, TAS_RL_data_path, RecRLexp_data_path, plot_path, ablation=True)

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--configure-env', default='none', help='Set test environment to setup all configuration')
    parser.add_argument('--exp-data-dir', default='/Experimental_Data/FEB14', help='Set experiment data location')
