...
#   shield_threshold = [.10, .20, .30, 0.40, .50, .60, .70, .80, .90, 1, 1.10, 1.20, 1.30, 1.40, 1.50, 1.60, 1.70, 1.80, 1.90, 2.00]
  ...
  TAS_RL_data_path = os.path.join(expdata_path, env_name, 'TAS_RL','different_shield_threshold')
  Safety_data_vec = []
  success_safety_vec = []
  #====================================================================
  for threshold in shield_threshold:
        ...
        atk_rate = 0.50
        epsilon = 0.50
        logdir = os.path.join(TAS_RL_data_path,'Threshold_{}'.format(threshold))
...
#   print(Safety_data_vec)
  ...
  plt.savefig(TAS_RL_data_path+'/safety_wrt_different_threshold.png',dpi=300,format='png',bbox_inches='tight')
