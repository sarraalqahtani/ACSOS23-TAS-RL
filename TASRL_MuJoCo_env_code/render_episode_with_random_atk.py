...
def run(env_name=None, eval_epi_no=100):
  ...
  TAS_RL_data_path = os.path.join(expdata_path, env_name, 'TAS_RL','render' ,'shield_threshold_{}'.format(shield_threshold))
  #====================================================================
  for eps in epsilon_list:
        atk_rate = eps
        epsilon = eps
        logdir = os.path.join(TAS_RL_data_path ,'atk_rate_{}_eps_{}'.format(atk_rate, epsilon))
        if not os.path.exists(logdir):
                os.makedirs(logdir)
        #**********************************************************************************************************
        for i in tqdm(range(eval_epi_no)):
              im_list_TAS_RL = render_run_eval_episode(env, env_name, expert_agent, safety_agent, shield_threshold=shield_threshold, atk_rate=atk_rate, epsilon=epsilon, render_function=render_function, sample=5)
              gif_path = logdir+'/TAS_RL/'
              npy_to_gif(im_list_TAS_RL, gif_path)
        ...
        RecRLexp_data_dir_sub_folders = os.path.join(RecRLexp_data_path,'render','Atk_rate{}_eps{}'.format(atk_rate, epsilon))
        if not os.path.exists(RecRLexp_data_dir_sub_folders):
             os.makedirs(RecRLexp_data_dir_sub_folders)
        recRL_experiment_data = run_comparison(env=env, env_name=env_name, rend_fun=render_function,  env_model_path= RecRL_model_path, s_rate= sample_rate, atk_rt=atk_rate, eps=epsilon, eval_episode= eval_epi_no, device=device)
